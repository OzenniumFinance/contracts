...
// added functionnalities by the ozennium's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when OZO mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ozoReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Withdraw OZO tokens from STAKING.
